...
<article class="flex flex-col items-center gap-4 w-full md:w-[30%]">
    <img class="w-full h-48 object-cover rounded-lg" src="<?= $livre->getUrl_image() ?>" alt="PP DU LIVRE">
    <p>
...
<article class="bg-[#26B6D9] p-6 w-full md:w-[60%] rounded-lg flex flex-col gap-4 items-center">
    ...
    <h3 class="text-center text-2xl font-semibold mt-8 mb-4 w-full">
        <?= $livre->getId_seller()->getNom() ?> vend également
    </h3>
    ...
    ?>
        <article class="bg-[#D9D9D9] opacity-65 p-6 mx-auto max-w-full sm:max-w-[48%] md:max-w-[30%] rounded-lg flex flex-col gap-4 items-center">
            <img src="<?= $livre->getUrl_image() ?>" alt="PP DU LIVRE" class="w-full h-48 object-cover rounded-lg">
            <p>
                Titre : <?= $livre->getTitre() ?>
            </p>
            <p>
